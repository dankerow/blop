...
export default class Triggered extends Command {
  constructor(client: Blop) {
    super(client, {
      _filename: import.meta.url,
      name: 'triggered',
      description: () => 'Generates a triggered GIF using your avatar.',
      cooldown: 5000,
      options: [
        {
          name: 'user',
  ...
  async execute({ client, interaction }: CommandContext) {
    ...
    return { files: [{ attachment: imageBuffer, name: `${this.name}.gif` }] }
  }
}
